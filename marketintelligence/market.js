...
function renderDrivers(data){
  ...
  el.innerHTML = data.drivers.map(d => {
    const text = `<span class="emoji">${d.emoji}</span><span>${d.text}</span>`;
    return d.url
      ? `<a class="driver-row" href="${d.url}" target="_blank" rel="noopener noreferrer">${text}</a>`
      : `<div class="driver-row">${text}</div>`;
  }).join('');
}
...
function renderEvents(data){
  ...
  el.innerHTML = data.events.map(e => {
    ...
    return e.url
      ? `<a class="event-row" href="${e.url}" target="_blank" rel="noopener noreferrer">${inner}</a>`
